...
- Reusable workflow: [`deploy-to-vercel`](.github/workflows/deploy-to-vercel.md)
  ...
  - Purpose: production deploy to Vercel via the CLI (`vercel pull` / `build` / `deploy --prebuilt`) using `VERCEL_TOKEN`, with optional GitHub Environment metadata (`deployment_environment`, `deployment_url`) so runs show under repo Environments / Deployments.
